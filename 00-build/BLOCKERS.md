...
> - **SC-11 — ⚠️ ANSWER-KEY EDIT, NEEDS YOUR SIGN-OFF.** `01-seed-cases/SC-11.answer-key.json` `detail_withheld` gained a lay restatement of facts *already stated clinically* (below target range / not responding / dose increased). No facts changed, `expected_critical_fail` untouched, ajv-validated. Also fixes the S4 validator's same ≥2-marker check. **Review the one-line diff before it's treated as approved.**
