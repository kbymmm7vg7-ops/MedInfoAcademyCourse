...
--                             body text lives in srd_document_bodies (split by
--                             migration 0007_answer_key_isolation.sql)
--   2. case_templates      — 12 seed cases (SC-01 .. SC-12); ground_truth_json
--                             (and, for SC-03, scripted_transcript_json) live in
--                             case_answer_keys (split by migration 0007), values
--                             transcribed verbatim from 01-seed-cases/*.answer-key.json
--
-- Safe to re-run: srd_documents/srd_document_bodies and case_templates/
-- case_answer_keys are each upserted by natural key (srl_code / case_code,
-- resolved to the referencing template_id or document_id). This file must NOT
-- set case_answer_keys.persona_brief_json — that column is owned by
-- seed_s3_persona_briefs.sql and would otherwise be clobbered on re-run.
-- =========================================================================
...
insert into srd_documents (org_id, srl_code, title, therapeutic_area, is_decoy_eligible)
values (
  null,
  'SRL-CDZ-INR',
...
Fictional training document — not medical advice.$gt$ from srd_documents where srl_code = 'SRL-CDZ-INR' and org_id is null
...
Fictional training document — not medical advice.$gt$ from srd_documents where srl_code = 'SRL-CDZ-RENAL' and org_id is null
on conflict (document_id) do update set body = excluded.body, updated_at = now();
...
Fictional training document — not medical advice.$gt$ from srd_documents where srl_code = 'SRL-CDZ-BLEED' and org_id is null
on conflict (document_id) do update set body = excluded.body, updated_at = now();
...
Fictional training document — not medical advice.$gt$ from srd_documents where srl_code = 'SRL-PUL-ACUTE' and org_id is null
on conflict (document_id) do update set body = excluded.body, updated_at = now();
...
insert into srd_document_bodies (document_id, body)
select id, $gt$Pulmonara (fesaterol) contains an inhaled corticosteroid component, and local deposition of corticosteroid in the mouth and throat during inhalation can predispose patients to oropharyngeal candidiasis (oral thrush). The Prescribing Information identifies oral candidiasis as a common local adverse reaction associated with inhaled corticosteroid-containing products in this class, and simple technique measures can meaningfully reduce this risk.
...
Fictional training document — not medical advice.$gt$ from srd_documents where srl_code = 'SRL-PUL-CANDID' and org_id is null
on conflict (document_id) do update set body = excluded.body, updated_at = now();
...
Fictional training document — not medical advice.$gt$ from srd_documents where srl_code = 'SRL-PUL-PEDS' and org_id is null
on conflict (document_id) do update set body = excluded.body, updated_at = now();
...
Fictional training document — not medical advice.$gt$ from srd_documents where srl_code = 'SRL-NEU-RASH' and org_id is null
on conflict (document_id) do update set body = excluded.body, updated_at = now();
...
insert into srd_document_bodies (document_id, body)
select id, $gt$Neurovance (melotigine) is indicated for the treatment of partial-onset seizures and requires a gradual titration schedule to the target maintenance dose, as described in the Prescribing Information's Dosage and Administration section. Initiating at the full maintenance dose without titration is not recommended and has been associated with a higher incidence of dose-related adverse reactions, including dizziness, somnolence, and coordination difficulty.
...
Fictional training document — not medical advice.$gt$ from srd_documents where srl_code = 'SRL-NEU-TITR' and org_id is null
on conflict (document_id) do update set body = excluded.body, updated_at = now();
...
Fictional training document — not medical advice.$gt$ from srd_documents where srl_code = 'SRL-NEU-PREG' and org_id is null
on conflict (document_id) do update set body = excluded.body, updated_at = now();
...
Fictional training document — not medical advice.$gt$ from srd_documents where srl_code = 'SRL-DRM-APPLY' and org_id is null
on conflict (document_id) do update set body = excluded.body, updated_at = now();
...
Fictional training document — not medical advice.$gt$ from srd_documents where srl_code = 'SRL-DRM-MALIG' and org_id is null
on conflict (document_id) do update set body = excluded.body, updated_at = now();
...
Fictional training document — not medical advice.$gt$ from srd_documents where srl_code = 'SRL-GAS-LONG' and org_id is null
on conflict (document_id) do update set body = excluded.body, updated_at = now();
...
Fictional training document — not medical advice.$gt$ from srd_documents where srl_code = 'SRL-GAS-ONSET' and org_id is null
on conflict (document_id) do update set body = excluded.body, updated_at = now();
...
Fictional training document — not medical advice.$gt$ from srd_documents where srl_code = 'SRL-OST-HYPOCAL' and org_id is null
on conflict (document_id) do update set body = excluded.body, updated_at = now();
...
Fictional training document — not medical advice.$gt$ from srd_documents where srl_code = 'SRL-OST-ONJ' and org_id is null
on conflict (document_id) do update set body = excluded.body, updated_at = now();
...
Fictional training document — not medical advice.$gt$ from srd_documents where srl_code = 'SRL-OST-PREG' and org_id is null
on conflict (document_id) do update set body = excluded.body, updated_at = now();
...
Fictional training document — not medical advice.$gt$ from srd_documents where srl_code = 'SRL-IMM-INFECT' and org_id is null
on conflict (document_id) do update set body = excluded.body, updated_at = now();
...
Fictional training document — not medical advice.$gt$ from srd_documents where srl_code = 'SRL-IMM-VACC' and org_id is null
on conflict (document_id) do update set body = excluded.body, updated_at = now();
...
insert into case_templates (
  ...
  product_ref, is_fictional_product, seed_or_generated,
...
insert into case_answer_keys (template_id, ground_truth_json)
select id, $gt${"case_id":"SC-01","title":"Cardizan & INR (clean inquiry)","difficulty_tier":1,"channel":"voice","therapeutic_area":"anticoagulation","inquiry_category":"drug_interactions","inquirer_contact":{"name":"Dr. Amara Chen","background":"family physician (HCP)","phone":"(555) 0182-4471","address":"88 Winslow Medical Plaza, Ste 210, Asheville, NC 28801"},"products":[{"name":"Cardizan","is_fictional":true,"srl_id":"SRL-CDZ-INR"}],"requester":{"type":"hcp","solicited":false},"safety":{"ae_present":true,"ae_four_elements_met":true,"ae_description":"Intercurrent adverse event: the patient on Cardizan has developed a sinus infection (an antibiotic is being prescribed for it); patient otherwise stable and asymptomatic. Captured for PV to assess regardless of suspected causality — an intercurrent illness reported on therapy is a reportable AE.","pc_present":false,"pregnancy_or_lactation":false,"special_situations":["none"],"dual_routing_required":false,"correct_routes":["pv"]},"compliance":{"off_label_involved":false,"medical_advice_risk":false},"correct_srl":"SRL-CDZ-INR","decoy_srl_ids":["SRL-CDZ-RENAL"],"documentation":{"required_fields":["requester_type","hcp_contact","product","inquiry_summary","ae_description","srl_cited","response_route"]},"reveal_rules":[{"cue":"No symptom cue exists; the patient is stable and no offhand mention is volunteered.","detail_withheld":"none — there is no AE to surface","surfaces_when":"if the trainee clarifies, the persona truthfully confirms the patient is fine and no event has occurred","volunteers_cue":false}],"expected_outcome":{"applicable_sections":["s1","s2","s4","s5"],"gold_result":"pass","common_failures":[{"description":"Answers the interaction question only; the sinus infection AE is missed entirely (never captured or routed)","expected_critical_fail":["S2.1"]},{"description":"Fabricates symptoms beyond the reported sinus infection (over-flagging)","expected_critical_fail":[]},{"description":"Trainee selects SRL-CDZ-RENAL decoy instead of SRL-CDZ-INR","expected_critical_fail":[]}]}}$gt$::jsonb from case_templates where case_code = 'SC-01'
...
insert into case_answer_keys (template_id, ground_truth_json)
select id, $gt${"case_id":"SC-02","title":"Gastroquell onset (ambiguous requester)","difficulty_tier":2,"channel":"voice","therapeutic_area":"GI","inquiry_category":"efficacy","inquirer_contact":{"name":"Jordan Ellis","background":"patient","phone":"(555) 0143-9920","address":"Dayton, OH"},"products":[{"name":"Gastroquell","is_fictional":true,"srl_id":"SRL-GAS-ONSET"}],"requester":{"type":"ambiguous","true_type_if_ambiguous":"patient","solicited":false},"safety":{"ae_present":true,"ae_four_elements_met":true,"ae_description":"Potential lack of effect: patient reports Gastroquell isn't working right away for their heartburn — no symptom relief yet on the current regimen, and the lack of relief is ongoing at the time of the call. Captured for PV to assess; flagging is not concluding the drug failed.","pc_present":true,"pc_description":"Potential product quality complaint associated with the reported lack of effect (product not delivering the expected effect); lot number, expiration date, NDC, and sample availability to be obtained.","pregnancy_or_lactation":false,"special_situations":["lack_of_effect"],"dual_routing_required":true,"correct_routes":["pv","quality"]},"compliance":{"off_label_involved":false,"medical_advice_risk":true,"off_label_correct_handling":"Not applicable (no off-label ask); must not advise a dose change — direct patient to HCP/pharmacist for regimen questions."},"correct_srl":"SRL-GAS-ONSET","decoy_srl_ids":["SRL-GAS-LONG","SRL-CDZ-INR"],"sop_timeframe_business_days":1,"documentation":{"required_fields":["requester_type","patient_contact","product","inquiry_summary","ae_description","pc_description","lot_number","expiration_date","ndc","suspect_product_availability","special_situation_flags","dual_routing_flags","srl_cited","response_route"]},"reveal_rules":[{"cue":"Caller uses clinical-adjacent terms (\"PPI\", \"acid suppression\") that could be mistaken for HCP language.","detail_withheld":"caller is actually the patient, not an HCP","surfaces_when":"trainee asks a requester-clarifying question (e.g. 'are you a healthcare professional, or are you taking this yourself?')","volunteers_cue":false}],"expected_outcome":{"applicable_sections":["s1","s2","s3","s4","s5"],"gold_result":"pass","common_failures":[{"description":"Treats caller as HCP and captures HCP contact fields","expected_critical_fail":[]},{"description":"Tells patient to double the dose","expected_critical_fail":["S4.2"]},{"description":"Selects SRL-GAS-LONG decoy","expected_critical_fail":[]},{"description":"Answers the onset/efficacy question but never captures the reported lack of effect (no AE/PC captured, no routing)","expected_critical_fail":["S2.1","S3.1","S5.2"]}]}}$gt$::jsonb from case_templates where case_code = 'SC-02'
on conflict (template_id) do update set
  ground_truth_json = excluded.ground_truth_json,
  updated_at = now();
...
insert into case_answer_keys (template_id, ground_truth_json)
select id, $gt${"case_id":"SC-05","title":"Neurovance hostile caller / legal threat (escalation)","difficulty_tier":5,"channel":"voice","therapeutic_area":"neurology","inquiry_category":"safety","inquirer_contact":{"name":"Sam Rivera","background":"caregiver (adult child of pt. Lucia Rivera)","phone":"(555) 0158-6642","address":"27 Halewood Ct, Providence, RI 02906"},"products":[{"name":"Neurovance","is_fictional":true,"srl_id":"SRL-NEU-RASH"}],"requester":{"type":"caregiver","solicited":false},"safety":{"ae_present":true,"ae_four_elements_met":true,"ae_description":"Blistering rash with mouth sores, hospitalized, possible SJS; onset ~2 weeks after starting Neurovance. Serious AE.","pc_present":false,"pregnancy_or_lactation":false,"special_situations":["none"],"dual_routing_required":false,"correct_routes":["pv","legal","media"]},"compliance":{"off_label_involved":false,"medical_advice_risk":true,"off_label_correct_handling":"Not applicable (no off-label ask); must not confirm causation, must not advise stopping/continuing the drug, must not admit fault — refer to treating physician/ER."},"correct_srl":"SRL-NEU-RASH","decoy_srl_ids":["SRL-NEU-TITR","SRL-DRM-APPLY","SRL-NEU-PREG"],"sop_timeframe_business_days":1,"documentation":{"required_fields":["requester_type","caregiver_contact","patient_identifier","product","inquiry_summary","ae_description","ae_onset","seriousness_flag","escalation_notes","srl_cited","response_route"]},"reveal_rules":[{"cue":"Caller opens hot with the legal/media threat: \"Your drug put my mother in the hospital with a horrible rash. I'm calling a lawyer and the news.\"","detail_withheld":"blistering rash, mucosal involvement, hospitalization, onset ~2 weeks after starting Neurovance","surfaces_when":"trainee de-escalates with empathy and control (acknowledges feelings, avoids fault/causation language, gives concrete next steps), after which the caller provides the clinical details","volunteers_cue":true}],"expected_outcome":{"applicable_sections":["s1","s2","s4","s5"],"gold_result":"pass","common_failures":[{"description":"Admits the drug caused the rash / apologizes for a product defect","expected_critical_fail":["S4.2"]},{"description":"Treats the legal and media threats as a routine complaint — neither Legal nor Media routing selected","expected_critical_fail":[]},{"description":"De-escalation fails (cold/defensive)","expected_critical_fail":[]},{"description":"Routes the AE to PV but omits the Legal/Media escalation","expected_critical_fail":[]}]}}$gt$::jsonb from case_templates where case_code = 'SC-05'
on conflict (template_id) do update set
  ground_truth_json = excluded.ground_truth_json,
  updated_at = now();
...
insert into case_answer_keys (template_id, ground_truth_json)
select id, $gt${"case_id":"SC-07","title":"Dermelia journalist inquiry (media edge case)","difficulty_tier":6,"channel":"voice","therapeutic_area":"dermatology","inquiry_category":"safety","inquirer_contact":{"name":"Alex Reyes","background":"health journalist, The Meridian Health Desk","phone":"(555) 0166-7788","address":"1400 Press Row, Rm 12, Chicago, IL 60601"},"products":[{"name":"Dermelia","is_fictional":true}],"requester":{"type":"journalist","solicited":false},"safety":{"ae_present":false,"pc_present":false,"pregnancy_or_lactation":false,"special_situations":["none"],"dual_routing_required":false,"correct_routes":["media"]},"compliance":{"off_label_involved":false,"medical_advice_risk":false},"correct_srl":"none","decoy_srl_ids":["SRL-DRM-MALIG","SRL-DRM-APPLY"],"sop_timeframe_business_days":1,"documentation":{"required_fields":["requester_type","journalist_contact","outlet","product","inquiry_summary","redirect_action","response_route"]},"reveal_rules":[{"cue":"Reporter opens asking MI to comment on Dermelia's cancer warning and presses for a quotable yes/no on causation.","detail_withheld":"none — the reveal is behavioral: whether the trainee avoids becoming an unauthorized spokesperson and avoids causation statements","surfaces_when":"not applicable — this is a behavioral compliance test, not an information-reveal","volunteers_cue":false}],"expected_outcome":{"applicable_sections":["s1","s4","s5"],"gold_result":"pass","common_failures":[{"description":"Gives a substantive comment / confirms or denies causation","expected_critical_fail":["S4.2"]},{"description":"Reads an SRL to the reporter as an official statement","expected_critical_fail":[]},{"description":"Doesn't capture outlet/contact or route to Media","expected_critical_fail":[]}]}}$gt$::jsonb from case_templates where case_code = 'SC-07'
on conflict (template_id) do update set
  ground_truth_json = excluded.ground_truth_json,
  updated_at = now();
...
insert into case_answer_keys (template_id, ground_truth_json)
select id, $gt${"case_id":"SC-08","title":"Dermelia embedded AE (volunteered cue, second therapeutic area)","difficulty_tier":3,"channel":"voice","therapeutic_area":"dermatology","inquiry_category":"other","inquirer_contact":{"name":"Taylor Brooks","background":"patient","phone":"(555) 0129-5514","address":"Boise, ID"},"products":[{"name":"Dermelia","is_fictional":true,"srl_id":"SRL-DRM-APPLY"}],"requester":{"type":"patient","solicited":false},"safety":{"ae_present":true,"ae_four_elements_met":true,"ae_description":"Persistent application-site burning and a spreading rash beyond the treated area, ~5 days, worsening.","pc_present":true,"pc_description":"Potential product quality complaint: persistent application-site burning with a rash spreading beyond the treated area on product use — possible product issue; lot number, expiration date, NDC, and sample availability to be obtained.","pregnancy_or_lactation":false,"special_situations":["none"],"dual_routing_required":true,"correct_routes":["pv","quality"]},"compliance":{"off_label_involved":false,"medical_advice_risk":true,"off_label_correct_handling":"Refer to prescriber for the worsening rash; advise stopping only per prescriber — MI does not direct treatment."},"correct_srl":"SRL-DRM-APPLY","decoy_srl_ids":["SRL-DRM-MALIG","SRL-GAS-LONG"],"sop_timeframe_business_days":1,"documentation":{"required_fields":["requester_type","patient_contact","product","inquiry_summary","ae_description","ae_onset","ae_worsening","concomitant_meds","medical_history","hcp_referral_note","pc_description","lot_number","expiration_date","ndc","suspect_product_availability","dual_routing_flags","srl_cited","response_route"]},"reveal_rules":[{"cue":"Persona volunteers offhand in beat 2: \"It stings a bit when I put it on, but creams do that, right?\"","detail_withheld":"persistent application-site burning + spreading rash, worsening over ~5 days","surfaces_when":"trainee catches the volunteered 'stings' cue and clarifies it","volunteers_cue":true}],"expected_outcome":{"applicable_sections":["s1","s2","s3","s4","s5"],"gold_result":"pass","common_failures":[{"description":"Lets the 'stings' cue pass without clarifying; AE missed entirely","expected_critical_fail":["S2.1","S3.1"]},{"description":"Advises patient to stop the drug on MI's own authority","expected_critical_fail":["S4.2"]},{"description":"AE identified but not documented","expected_critical_fail":[]}]}}$gt$::jsonb from case_templates where case_code = 'SC-08'
on conflict (template_id) do update set
  ground_truth_json = excluded.ground_truth_json,
  updated_at = now();
...
insert into case_answer_keys (template_id, ground_truth_json)
select id, $gt${"case_id":"SC-10","title":"Neurovance pregnancy exposure (special situation)","difficulty_tier":4,"channel":"voice","therapeutic_area":"neurology","inquiry_category":"safety","inquirer_contact":{"name":"Dana Whitfield","background":"patient","phone":"(555) 0172-8830","address":"Frederick, MD"},"products":[{"name":"Neurovance","is_fictional":true,"srl_id":"SRL-NEU-PREG"}],"requester":{"type":"patient","solicited":false},"safety":{"ae_present":true,"ae_four_elements_met":true,"ae_description":"Pregnancy exposure (reportable special situation entered as a safety report): patient approximately 6 weeks pregnant by last menstrual period with Neurovance exposure throughout; no adverse outcome reported. Exposure ongoing at the time of the call.","pc_present":false,"pregnancy_or_lactation":true,"special_situations":["pregnancy_exposure"],"dual_routing_required":false,"correct_routes":["pv"]},"compliance":{"off_label_involved":false,"medical_advice_risk":true,"off_label_correct_handling":"Not applicable (no off-label ask); must NOT tell patient to stop or continue an anticonvulsant (abrupt discontinuation risk) — refer urgently to prescriber."},"correct_srl":"SRL-NEU-PREG","decoy_srl_ids":["SRL-NEU-TITR","SRL-OST-PREG","SRL-DRM-APPLY"],"sop_timeframe_business_days":1,"documentation":{"required_fields":["requester_type","patient_contact","product","inquiry_summary","ae_description","special_situation_flags","estimated_gestational_age_or_lmp","prescriber_info","registry_referral","no_treatment_advice_note","srl_cited","response_route"]},"reveal_rules":[{"cue":"The pregnancy exposure is stated up front, not hidden: \"I just found out I'm 6 weeks pregnant and I've been on Neurovance the whole time — should I stop it?\"","detail_withheld":"none — the skill is recognizing it as a reportable special situation and handling the 'should I stop' trap correctly, not surfacing a hidden fact","surfaces_when":"if the trainee clarifies, patient confirms no current adverse outcome to the pregnancy or mother, and provides LMP/estimated gestational age and prescriber info if asked","volunteers_cue":true}],"expected_outcome":{"applicable_sections":["s1","s2","s4","s5"],"gold_result":"pass","common_failures":[{"description":"Tells the patient to stop (or keep taking) Neurovance","expected_critical_fail":["S4.2"]},{"description":"Doesn't flag the pregnancy exposure / doesn't route to PV / omits registry","expected_critical_fail":["S2.1","S5.2"]},{"description":"Fabricates an AE where there is only an exposure","expected_critical_fail":[]}]}}$gt$::jsonb from case_templates where case_code = 'SC-10'
on conflict (template_id) do update set
  ground_truth_json = excluded.ground_truth_json,
  updated_at = now();
...
insert into case_answer_keys (template_id, ground_truth_json)
select id, $gt${"case_id":"SC-11","title":"Cardizan disposal question, embedded lack-of-effect (LOE)","difficulty_tier":3,"channel":"voice","therapeutic_area":"anticoagulation","inquiry_category":"other","inquirer_contact":{"name":"Marcus Bell","background":"patient","phone":"(555) 0121-6675","address":"Chattanooga, TN"},"products":[{"name":"Cardizan","is_fictional":true}],"requester":{"type":"patient","solicited":false},"safety":{"ae_present":true,"ae_four_elements_met":true,"ae_description":"Potential lack of effect: on the lowest Cardizan dose for about six weeks the INR stayed below the target range at every check and was not responding to that dose, until the doctor increased it (subtherapeutic anticoagulation). No clot or bleed reported; resolved from the caller's perspective after the prescriber increased the dose. Captured for PV to assess; flagging is not concluding the drug failed.","pc_present":true,"pc_description":"Potential product quality complaint associated with the reported lack of effect on the lower-dose tablets; the patient still has leftover tablets — lot number, expiration date, NDC, and sample availability to be obtained.","pregnancy_or_lactation":false,"special_situations":["lack_of_effect"],"dual_routing_required":true,"correct_routes":["pv","quality"]},"compliance":{"off_label_involved":false,"medical_advice_risk":true,"off_label_correct_handling":"Not applicable (no off-label ask); must NOT advise on dosing or the INR target — refer all therapy questions to the prescriber/pharmacist. Disposal guidance is general safety info, not medical advice."},"correct_srl":"none","decoy_srl_ids":["SRL-CDZ-INR","SRL-CDZ-RENAL"],"sop_timeframe_business_days":1,"documentation":{"required_fields":["requester_type","patient_contact","product","inquiry_summary","ae_description","pc_description","lot_number","expiration_date","ndc","suspect_product_availability","special_situation_flags","dual_routing_flags","dose_titration_note","disposal_guidance_given","no_dosing_advice_note","response_route"]},"reveal_rules":[{"cue":"Persona volunteers in beat 2: \"My doctor bumped me up because the low dose wasn't getting my INR where it needed to be.\"","detail_withheld":"potential lack of effect — persistently sub-target INR over several weeks on the lowest Cardizan dose (subtherapeutic anticoagulation): at every INR check the readings stayed below the target range and were not responding to that dose, until the doctor increased it","surfaces_when":"trainee catches the cue and clarifies (how long on the low dose, what the INR was doing, any clotting symptoms)","volunteers_cue":true}],"expected_outcome":{"applicable_sections":["s1","s2","s3","s4","s5"],"gold_result":"pass","common_failures":[{"description":"Answers only the disposal question; dismisses the dose-increase remark as 'normal titration' and never captures the LOE","expected_critical_fail":["S2.1","S3.1","S5.2"]},{"description":"Advises the patient on dosing / the INR target / whether the increase was right","expected_critical_fail":["S4.2"]},{"description":"Selects SRL-CDZ-INR or SRL-CDZ-RENAL to answer a disposal question","expected_critical_fail":[]},{"description":"Over-states the LOE as an actual clot/bleed AE that didn't occur","expected_critical_fail":[]}]}}$gt$::jsonb from case_templates where case_code = 'SC-11'
on conflict (template_id) do update set
  ground_truth_json = excluded.ground_truth_json,
  updated_at = now();
